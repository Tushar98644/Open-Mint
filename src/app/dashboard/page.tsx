...
import OpenMint from '../../../blockchain/artifacts/contracts/OpenMint.sol/OpenMint.json';
...
export default function Dashboard(){
    ...
    const publishToken = async (e) => {
        if (typeof window.ethereum === "undefined") {
          toast.error("Please install MetaMask to interact with this application", {
            position: toast.POSITION.TOP_CENTER
          });
          return;
        }
      
        const providers = new Provider(window.ethereum);
        const signer = providers.getSigner();
        await providers.send("eth_requestAccounts", []); // Request wallet connection
        ...
        if (formInput.name === "") {
          ...
          try {
            ...
            const tokenData = await contract.createToken(
              tokenUri,
              formInput.supply,
              price,
              formInput.category
